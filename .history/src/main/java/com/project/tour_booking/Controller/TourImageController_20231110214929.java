...
public class TourImageController {
  ...
  public ResponseEntity<String> saveTourImage(@RequestBody TourImage tourImage) {
    List<TourImage> tourImages = tourImageService.geAlltTourImage();
    Long tourImageId = tourImage.getId();
    Long tourId = tourImage.getTour().getId();
    for (TourImage item : tourImages) {
      if (item.getId().equals(tourImageId) && item.getTour().getId().equals(tourId))
        return new ResponseEntity<>("ẢNH NÀY ĐÃ TỒN TẠI", HttpStatus.OK);
    }
