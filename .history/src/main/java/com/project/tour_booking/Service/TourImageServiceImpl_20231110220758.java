...
public class TourImageServiceImpl implements TourImageService {
  ...
  @Override
  public TourImage updateTourImage(TourImage tourImage) {
    Optional<TourImage> updateTourImage = tourImageRepository.findById(tourImage.getId());
    if (updateTourImage.isPresent()) {
      ...
      throw new TourImageNotFoundException(tourImage.getId());
  }
