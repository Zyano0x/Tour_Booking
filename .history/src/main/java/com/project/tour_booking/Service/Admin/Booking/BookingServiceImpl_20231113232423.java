...
public class BookingServiceImpl implements BookingService {
  ...
  public void updateStatusBooking(Long bookingId) {
    Booking updateStatusBooking = bookingRepository.findById(bookingId).get();
    Tour updateTour = updateStatusBooking.getTour();
    if (updateStatusBooking.getStatus() == true) {
      updateStatusBooking.setStatus(false);
      updateTour.setQuantity(updateTour.getQuantity()
          - (updateStatusBooking.getQuantityOfAdult() + updateStatusBooking.getQuantityOfChild()));
    } else {
      updateStatusBooking.setStatus(true);
      updateTour.setQuantity(updateTour.getQuantity() + 1);
    }
