...
public class TourServiceImpl implements TourService {
  ...
  public void saveTour(Tour tour) {
    ...
    tour.setTypeOfTour(typeOfTourRepository.findById(tour.getTotId()).get());
  ...
  @Override
  public void updateQuantityTour(Long tourId, Integer quantity, Boolean bookingStatus) {
    Optional<Tour> updateQuantityTourOptional = tourRepository.findById(tourId);
    if (updateQuantityTourOptional.isPresent()) {
      Tour updateQuantityTour = updateQuantityTourOptional.get();
      Integer tourQuantity = updateQuantityTour.getQuantity();
      if (bookingStatus == false)
        updateQuantityTour.setQuantity(tourQuantity + quantity);
      else
        updateQuantityTour.setQuantity(tourQuantity - quantity);
      tourRepository.save(updateQuantityTour);
    } else
      throw new TourNotFoundException(tourId);
  }
