...
public class TourImageServiceImpl implements TourImageService {
  ...
  public void saveTourImage(TourImage tourImage) {
    TourImage newTourImage = new TourImage(tourImage.getPath(), tourImage.getTourIdForUpdate());
    newTourImage.setTour(tourRepository.findById(tourImage.getTourIdForUpdate()).get());
  ...
  public TourImage updateTourImage(TourImage tourImage, Long tourImageId) {
    ...
    if (updateTourImage.isPresent()) {
      ...
      updateTourImage2.setTour(tourRepository.findById(tourImage.getTourIdForUpdate()).get());
